...
export function Sponsors({ content }: { content: HomeConfig['sponsors'] }) {
  ...
  const hasSponsors = sponsors.gold.length > 0 || sponsors.silver.length > 0 || sponsors.community.length > 0
      ...
      {hasSponsors ? (
        <div style={{ marginTop: '40px' }}>
          ...
          {sponsors.community.length > 0 && (
            <div style={{ marginBottom: '32px' }}>
              <p style={{ fontSize: '12px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '12px' }}>Community</p>
              <p style={{ fontSize: '14px', color: 'var(--text-secondary)' }}>
                {sponsors.community.map((s, i) => (
                  <span key={s.name}>
                    <a href={s.href} target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'none' }}>{s.name}</a>
                    {i < sponsors.community.length - 1 ? ' · ' : ''}
                  </span>
                ))}
              </p>
            </div>
        ...
        <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginTop: '16px', marginBottom: '32px' }}>
