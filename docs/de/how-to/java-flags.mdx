...
Eine korrekte Optimierung erfordert ein methodisches Vorgehen. **Beziehe dich auf die detaillierten Schritte im Guide ["Optimize Your Leaf Server"](../how-to/optimize-leaf-server), um zu verstehen, _wie_ diese grundlegenden Maßnahmen umzusetzen sind.** Zusammengefasst gilt immer:
...
- **Aktion:** **Vernachlässige diese Dateien nicht.** Sieh dir die Doku deiner Server-Software an. **Für detaillierte Hinweise zu Leaf-spezifischen Einstellungen (`leaf-global.yml`) siehe erneut den Guide ["Optimize Your Leaf Server"](../how-to/optimize-leaf-server).** Nutze Spark-Profilergebnisse, um zu erkennen, welche Gameplay-Aspekte (oft korreliert mit hoher Allokation) angepasst werden müssen.
